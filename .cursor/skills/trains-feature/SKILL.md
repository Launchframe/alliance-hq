...
- [ ] Composite templates: segment day index is calendar Tue=0…Mon=6 (`compositeSegmentDayIndex` in `week-template-registry.shared.ts`), not `trainWeekStartDow`. `takedown_week` → with-replacement PIF (`usesPriceIsFreightConductorRoll` in `heavy-hitter-pool.shared.ts` — not `isPriceIsRightPaintTemplate` alone).
