...
- [ ] Define new `CONDUCTOR_MECHANISMS` or `VIP_MECHANISMS` entries in `src/lib/trains/templates.ts` if adding a selection mechanism.
- [ ] Define new `WEEK_TEMPLATE_SEGMENTS` or composite `WEEK_TEMPLATES` entries in `src/lib/trains/week-template-registry.shared.ts` if adding a template.
- [ ] Add day-of-week rules and safeguards (e.g. no VS on Monday — no Sunday scores).
- [ ] If composite, define `segments` array with `weekdayTemplate` and optional `weekendTemplate`.
- [ ] Add the template to `SELECTABLE_WEEK_TEMPLATES` if it should appear in the template picker.
- [ ] Update `inferWeekTemplateFromDayConfigs` if the template infers from painted days.
...
- [ ] Classify as `PoolSpinSource`, `VsLeaderboardSpinSource`, `DonationsLeaderboardSpinSource`, or `ManualPickSource`.
- [ ] If pool-based, define pool type in `POOL_TYPES` and wire pool creation/seeding in `service.ts`.

## 3. Guided flow integration

- [ ] Update `conductorPoolKind()` in `guided-flow.shared.ts` to map the mechanism to its `GuidedPoolKind`.
- [ ] Update `resolvePoolStepState()` if the pool has custom readiness logic.
- [ ] Verify `currentGuidedStep()` blocks correctly for the new mechanism.
- [ ] Update `mechanismUsesWheelSpin()` — does this mechanism spin or only manual pick?
- [ ] If the mechanism has prerequisites (scores, roster), add the blocking CTA to the `prepare_pool` step in `TrainsGuidedConductorFlow.tsx`.

## 4. Dashboard wiring (both modes)

**Guided mode:**

- [ ] Add the action to the appropriate guided flow step (prepare_pool, conductor, vip, or a new step if needed).
- [ ] Render primary/secondary buttons per the mode contract (see `trains-simple-advanced-modes.mdc`).

**Advanced mode:**

- [ ] Add the equivalent control to the advanced mode layout in `TrainsDashboard.tsx`.
- [ ] If the feature has a prerequisite, add a hint banner in advanced mode.
...
- [ ] Add translation keys to `messages/en-US.json` under `Trains` namespace.
- [ ] Add corresponding keys to `messages/pt-BR.json`.
...
- [ ] Verify reel dedup logic handles the candidate list size (≥ 3 unique names for viewport dedup).
- [ ] If the mechanism is manual-pick only, verify `supportsManualVipPick` / `supportsManualConductorPick` includes it.
...
- [ ] Ensure RBAC: conductor mutations require `trains:write`; reads require `scores:read`.
...
- [ ] If the feature changes guided flow steps, update `.cursor/rules/trains-simple-advanced-modes.mdc`.
