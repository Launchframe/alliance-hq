...
- **i18n** — maintainer must approve English copy before `messages/en-US.*` or `messages/pt-BR.*` change (see [`.cursor/rules/user-facing-copy-review.mdc`](.cursor/rules/user-facing-copy-review.mdc)); then en-US + pt-BR; run `npm run i18n:validate`. Every user-visible surface (web, Discord, generated images) must use the active locale — see [`.cursor/rules/i18n-all-surfaces.mdc`](.cursor/rules/i18n-all-surfaces.mdc).
