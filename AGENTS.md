...
- VS scoreboard review lists sort by score descending (same order as the computed `#` ranks). Same-score OCR name twins collapse at parse time (`dedupeSameScoreOcrTwins`, similarity ≥ 0.75). Matched-member dropdowns keep already-assigned names searchable. Combobox focus selects the current label so typing replaces it.
