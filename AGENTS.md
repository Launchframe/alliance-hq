...
- Focus refreshes during explicit import-page navigation are coalesced and run against the resulting page's cursor after navigation settles; they must not supersede the pager request. Older background responses still use the existing read/revision fences.
...
- The worker child process strips `OCR_WORKER_SECRET`, DB URLs, and user site packages, but it does not run in a separate network namespace. Treat it as an isolation limit: deploy behind a broker or container with egress deny if untrusted models are loaded.
...
- VS scoreboard review lists sort by score descending (same order as the computed `#` ranks). Same-score OCR name twins collapse at parse time (`dedupeSameScoreOcrTwins`, similarity ≥ 0.75 on long strings or OCR-junk stems — not prefix containment). Two different member ids never merge. Matched-member dropdowns keep already-assigned names searchable. Combobox focus selects the current label so typing replaces it.
