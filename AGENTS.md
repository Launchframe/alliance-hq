...
- **Trains** — GET routes use `scores:read`; mutations use `trains:write` via `requireTrainOfficer`; tenant-scoped by session alliance; conductor lock is immutable without audited override; season + ritual detail in [`.cursor/rules/trains.mdc`](.cursor/rules/trains.mdc); day-vs-week paint, lock-before-VIP, swap/share/wizard UX in [`.cursor/rules/trains-conductor-ux.mdc`](.cursor/rules/trains-conductor-ux.mdc)
...
- History discovery cursors and list-row timestamps retain database microseconds and a stable ID tie-breaker. Bind timestamp strings as `::text::timestamptz`; postgres.js otherwise serializes a timestamp-typed parameter through JavaScript Date and loses precision. Cursors are scope-bound pagination metadata, never access grants.
- Focus refreshes during explicit import-page navigation are coalesced and run against the resulting page after navigation settles; they must not supersede the pager request or lose its previous-page stack. Older background responses still use the existing read/revision fences.
- Hidden import pagers mount their error anchor only when an error arrives. Scroll that anchor after the error state commits; a requestAnimationFrame scheduled from the catch handler can run before it exists.
- OCR history retains non-noise preamble as unattributed review rows even when a later sender header parses. Recognized-message locators stay stable. Redact before chunking orphan text, and keep the screenshot adapter server-only.
- Native OCR smoke uses a committed headerless PNG and asserts unknown sender/date plus explicit review requirements; it does not depend on runtime SVG fonts. The overall test timeout includes staging and assertions, while the process request is bounded to the route's 60-second limit.
...
- Compact task/board SQL bounds description and source-title text and omits provenance. Remove a truncated trailing token before shared redaction; blindly clipping an identifier can prevent its redaction. Selected-task loading/errors belong in a dialog even when its card is filtered out.
...
- HQ Notes use `knowledge_resources` ownership/grants in addition to route RBAC. Resolve web actors with `getKnowledgeActorForSession`; a workspace cookie alone, a linked commander, or an officer/maintainer role is not a private-note read grant. Notes queries and commander-profile projections must use the same resource predicate.
- A Notes capture keeps its original `web` / `discord` source and resource identity. The database trigger supports legacy-style inserts, and Discord-to-HQ ownership claiming is one-way: only `ownership_state = discord` can be claimed. Verified account merges must remap knowledge ownership and recipient grants before deleting the old HQ user.
- Zod 4 creation defaults also apply inside optional/partial fields. Build PATCH schemas from validators without creation defaults; otherwise a body-only edit can clear titles, member links, or other omitted fields. Notes has an explicit regression test for this.
- The E2E server runs `npm run build` (including migration and seeds) with its generated test environment and all three database URL variables bound to the guarded E2E database. This exercises the production build gate without using a developer or production database.
- Notes boards reuse `useVersionedSnapshot` and Postgres/SSE. Keep task-resource locks before sorted board locks; canonical task mutations must notify every affected board within the same transaction. An individual task grant never grants officer-board metadata or stream access.
- Legacy Officer Intel source sessions and meeting notes also have knowledge-resource envelopes. Pass the authenticated actor through every read, count, image lookup, and mutation; a shared derived note/task does not authorize its source. Records without proven creators remain unresolved and closed.
- Legacy Officer Intel Q&A/indexing is blocked until consent-aware evidence lineage is implemented. Approval alone does not authorize embedding, and source import does not automatically call translation providers. Do not restore alliance-wide corpus retrieval as a fallback.
- History imports seal checksum-verified original bytes under keys distinct from upload targets; never serve original keys through a read URL. Use strict streaming for untrusted objects, not the legacy whole-buffer fallback. New import requests bind the visible account/alliance scope before any ownership is created.
- History parsing checkpoints one bounded file at a time in the existing source-message store. Lock membership/permission before resource and job rows; lease tokens plus owner/content/access revisions fence completion. Staged and excluded messages must remain outside legacy source projections. Review and commit do not enable AI or sharing.
- Legacy meeting IDs map to canonical `performance_notes` via `canonical_note_id`; old summary/decision/question columns are frozen history, not writable content. Canonical documents include `body`, `keyDecisions`, and `openQuestions`; include all three in previews, revisions, search, and future AI/publication projections. Content edits invalidate legacy approval.
- Locale rewrites can leave the colon in `meeting:` note IDs encoded. `noteRouteId` normalizes that specific prefix at the Notes page boundary without decoding arbitrary paths.
- Knowledge approval binds `content_version`, not the general resource version. Indexing is a separate explicit action after owner consent. Retrieval requires a completed generation and current content/access/approval/consent/model lineage plus current reader/owner eligibility; legacy chunks without lineage remain excluded.
- Corpus fingerprints use ordered evidence tuples because Postgres JSONB changes object-key ordering. Do not hash a round-tripped JSONB object with order-sensitive JSON serialization.
- Drizzle single-table computed SELECTs can remove PgColumn qualifiers inside correlated subqueries. Knowledge projections use explicit qualified SQL references to avoid treating an inner row ID as the outer resource ID.
