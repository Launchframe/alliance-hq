...
- Commander name conflicts are not a roster-sync gate. `syncCommanderFromAllianceMember` reattaches a stale Ashed id when no other **active** roster row owns the name, and skips the conflict when this roster row is already linked. Members refresh must not auto-open the resolve sheet; a name save reloads `/api/members` **without** `refresh=1` so Ashed cannot write the old display name back and reopen the dialog.
