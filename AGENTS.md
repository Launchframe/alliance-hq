...
- **Native alliance invites** — `createHqInvite` (team settings + `/api/admin/native-alliances/.../invites`): **owner** invites are allowed before a linked game server; **officer/member** invites require `alliances.game_server_id` → `game_servers` (set automatically when the owner completes name+UID member link, or via admin console / Game season for corrections). API returns **422** `{ code: "alliance_server_required" }` when non-owner invites lack a linked server. **Commander claim invites** are member-role invites with `targetAshedMemberId` (bulk via `createHqClaimInvitesBulk`). Operator guides: `/guides/alliance-onboarding`; agent rule: `.cursor/rules/native-alliance-invites-rbac.mdc`.
