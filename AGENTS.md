...
- **VS daily announcement cron:** `/api/internal/vs/daily-announcement` runs `50 1 * * *` UTC (~23:50 game server time, UTC−2 — just before nightly reset) and posts a preview of **tomorrow's** VS day (`targetDate = server-today + 1`) to each opted-in alliance's `vs_announcements_channel_id` (falls back to `regular_events_channel_id`). Opt-in is `alliances.vs_announcements_enabled` (auto-enabled by `/set-regular-events-channel`); idempotent per alliance+date via `vs_announcement_posts`. Locale is resolved per alliance (majority `/language` preference among linked R4+ officers, falling back to any linked member, default `en-US`) since there is no guild-level locale column.
