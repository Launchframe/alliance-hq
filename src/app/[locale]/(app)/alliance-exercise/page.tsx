...
export async function generateMetadata() {
  ...
  return await allianceScopedMetadata(t("allianceExercise"));
}

export default async function AllianceExercisePage() {
  const locale = await getLocale();
  const session = await requirePageSession("/alliance-exercise");
  const state = await getSessionStateFor(session, locale);
  if (!state.canUseAshedEmbeds) {
    redirect({ href: "/members", locale });
  }

  const route = resolveIframePage("alliance-exercise");
