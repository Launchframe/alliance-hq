...
import { AllianceVsMembershipSettings } from "@/components/settings/AllianceVsMembershipSettings";
...
export default async function SettingsVsCompliancePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  const { locale } = await params;
  const session = await requirePageSession("/settings/vs-compliance");
  ...
  const t = await getTranslations("settings.vsCompliancePage");
  ...
  return (
    <div className="mx-auto w-full min-w-0 max-w-lg space-y-6">
      ...
      <AllianceVsMembershipSettings allianceTag={allianceTag} />
