...
export async function POST(request: Request, { params }: Props) {
  ...
  if (target.id === "vs-performance" && batchesOnDate.length && batchesOnDate.every(isLocalVsBatch)) return changeLocalVsData({ allianceId: ctx.allianceId, rbac: ctx.rbac, batches: batchesOnDate, wholeDate: recordedDate, newRecordedDate });
