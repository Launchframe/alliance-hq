...
import { cancelDraftSchema } from "@/lib/support-teams/draft-api.shared";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { idempotencyKey, ...input } = cancelDraftSchema.parse(await request.json());
    return privateJson(await executeDraftCommand(access, { ...input, kind: "cancelDraft", draftId: (await context.params).id }, idempotencyKey));
