...
import { pickDraftSchema } from "@/lib/support-teams/draft-api.shared";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { idempotencyKey, ...input } = pickDraftSchema.parse(await request.json());
    return privateJson(await executeDraftCommand(access, { ...input, kind: "draftPick", draftId: (await context.params).id }, idempotencyKey));
