...
export function CalendarConnectionsClient({ initial }: { initial: CalendarSettingsData }) {
  ...
  const [actionError, setActionError] = useState<ActionError>(null), [reveal, setReveal] = useState<{ id: string; version: number } | null>(null);
  ...
  async function mutate(body: unknown, scope: string) {
  ...
  async function savePreferences() {
    try {
      ...
      return await mutate({ action: "preferences", version: data.preferences.version, preferences }, "preferences");
    } catch { fail("preferences", "invalidPreferences"); return null; }
  }
  async function configure(change: TargetChange) {
    if (change.enabled && !data.preferences.version && !await savePreferences()) return;
    const saved = await mutate({ action: "target", ...change }, `${change.allianceId}:${change.provider}`);
    if (saved && alive.current) {
      const target = saved.targets.find((row) => row.allianceId === change.allianceId && row.provider === change.provider);
      setReveal(change.rotate && target ? { id: target.id, version: target.version } : null);
      setConfirmation(null); heading.current?.focus();
    }
  }
  async function showPreview(allianceId: string, sources: CalendarSource[], scope: string) {
    previewRequest.current?.abort();
    const controller = new AbortController(); previewRequest.current = controller;
    setPreviewBusy(true); setMessage(""); setActionError(null); setPreview(null);
    try {
  ...
  async function connectGoogle() {
    ...
    setBusy(true); setMessage(""); setActionError(null);
    try {
  ...
  function confirm(target: Target, rotate: boolean) { setCleanup(false); setMessage(""); setActionError(null); setConfirmation({ target, rotate }); }
  ...
  return <main className="mx-auto w-full min-w-0 max-w-4xl space-y-6 p-4">
    ...
    {message === "saved" && <p role="status" className="text-hq-fg">{t(message)}</p>}
    <form aria-label={t("alerts")} className="space-y-4 rounded-xl border border-hq-border bg-hq-surface p-4" onSubmit={(event) => { preventDefaultFormSubmit(event); void savePreferences(); }}>
    ...
    {data.googleAvailable && <section className="space-y-2 rounded-xl border border-hq-border bg-hq-surface p-4"><h2 className="text-lg font-semibold">{t("google")}</h2>{data.account && <p>{data.account.email}</p>}{data.account?.status === "connected" ? <p role="status">{t("status.connected")}</p> : <button className={button} disabled={busy} onClick={() => void connectGoogle()}>{t("connect")}</button>}<CalendarActionError error={actionError} scope="google" /></section>}
    ...
    {data.alliances.map((alliance) => <section key={alliance.id} className="space-y-3"><h2 className="text-lg font-semibold">{alliance.tag || alliance.name}</h2>{(["apple", ...(data.account?.status === "connected" ? ["google"] : [])] as const).map((provider) => {
      ...
      return <CalendarTargetCard key={`${provider}:${target?.version ?? 0}`} alliance={alliance} provider={provider} target={target} busy={busy} actionError={actionError} revealLink={!!target && reveal?.id === target.id && reveal.version === target.version} onSave={configure} onPreview={(id, sources) => showPreview(id, sources, scope)} onConfirm={confirm} />;
    })}</section>)}
    ...
    <dialog ref={dialog} role="dialog" aria-modal={confirmation ? true : undefined} aria-label={t(confirmation?.rotate ? "rotate" : "disconnect")} className="max-h-[90vh] w-[min(95vw,36rem)] space-y-4 overflow-auto rounded-xl border border-hq-border bg-hq-canvas p-5 text-hq-fg backdrop:bg-black/60" onCancel={(event) => { if (busy) event.preventDefault(); else setConfirmation(null); }}>
      ...
      {actionError && <CalendarActionError error={actionError} scope={actionError.scope} />}
...
function CalendarActionError({ error, scope }: { error: ActionError; scope: string }) {
  const t = useTranslations("calendarConnections"), ref = useRef<HTMLParagraphElement>(null);
  const message = error?.scope === scope ? error.message : "";
  useEffect(() => { if (message) ref.current?.scrollIntoView({ block: "nearest" }); }, [message]);
  return message ? <p ref={ref} role="alert" className="text-sm text-hq-danger">{t(message)}</p> : null;
}

async function loadPrivateLink(targetId: string, signal?: AbortSignal): Promise<string> {
  const response = await fetch("/api/calendar/feed-link", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ targetId }), signal });
  const data = await response.json();
  if (!response.ok || typeof data.url !== "string") throw new Error();
  return data.url;
}

function CalendarTargetCard({ alliance, provider, target, busy, actionError, revealLink, onSave, onPreview, onConfirm }: { alliance: Alliance; provider: string; target?: Target; busy: boolean; actionError: ActionError; revealLink: boolean; onSave: (change: TargetChange) => Promise<void>; onPreview: (id: string, sources: CalendarSource[]) => Promise<void>; onConfirm: (target: Target, rotate: boolean) => void }) {
  ...
  useEffect(() => {
    ...
    void loadPrivateLink(targetId, controller.signal).then((link) => { if (!controller.signal.aborted) setLink(link); }).catch(() => { if (!controller.signal.aborted) setError(true); }).finally(() => { if (!controller.signal.aborted) setLinkBusy(false); });
    return () => controller.abort();
  }, [revealLink, targetId]);
  async function showLink() {
    if (!targetId || linkBusy) return;
    setLinkBusy(true); setError(false);
    try { setLink(await loadPrivateLink(targetId)); } catch { setError(true); } finally { setLinkBusy(false); }
  }
  return <section aria-label={`${alliance.tag || alliance.name} — ${t(provider === "apple" ? "apple" : "google")}`} className="space-y-3 rounded-xl border border-hq-border bg-hq-surface p-4">
    <form className="space-y-3" onSubmit={(event) => { preventDefaultFormSubmit(event); void onSave({ allianceId: alliance.id, provider, sources, enabled, version: target?.version ?? 0 }); }}>
