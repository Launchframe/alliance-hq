...
export function CalendarConnectionsClient({ initial, initialError = false }: { initial: CalendarSettingsData; initialError?: boolean }) {
  ...
  async function connectGoogle() {
    ...
    setBusy(true); setMessage(""); setActionError(null); inFlight.current = true;
    try {
  ...
  function openConfirmation(value: CalendarConfirmation) { setCleanup(false); setMessage(""); setActionError(null); setConfirmation(value); }
  function confirm(target: Target, rotate: boolean, reset?: boolean) { openConfirmation({ kind: "target", target, rotate, reset }); }
  ...
  async function confirmChange() {
    if (confirmation?.kind === "account") {
      const saved = await mutate({ version: confirmation.version, cleanup }, "google", "/api/calendar/google/disconnect");
  ...
  return <main className="mx-auto w-full min-w-0 max-w-4xl space-y-6 p-4">
    ...
    {(data.googleAvailable || data.account || initialError) && <section aria-label={t("google")} className="space-y-2 rounded-xl border border-hq-border bg-hq-surface p-4">
      ...
      <CalendarActionError error={confirmation ? null : actionError} scope="google" />
    ...
    <dialog ref={dialog} role="dialog" aria-modal={confirmation ? true : undefined} aria-label={t(confirmationKey)} className="max-h-[90vh] w-[min(95vw,36rem)] space-y-4 overflow-auto rounded-xl border border-hq-border bg-hq-canvas p-5 text-hq-fg backdrop:bg-black/60" onCancel={(event) => { if (busy) event.preventDefault(); else setConfirmation(null); }}>
      ...
      {confirmation && actionError && <CalendarActionError error={actionError} scope={actionError.scope} />}
...
function CalendarTargetCard({ alliance, provider, target, busy, canEnable, timezone, actionError, revealLink, onSave, onPreview, onConfirm }: { alliance: Alliance; provider: string; target?: Target; busy: boolean; canEnable: boolean; timezone: string; actionError: ActionError; revealLink: boolean; onSave: (change: TargetChange) => Promise<void>; onPreview: (id: string, sources: CalendarSource[]) => Promise<void>; onConfirm: (target: Target, rotate: boolean, reset?: boolean) => void }) {
  ...
  return <section aria-label={`${alliance.tag || alliance.name} — ${t(provider === "apple" ? "apple" : "google")}`} className="space-y-3 rounded-xl border border-hq-border bg-hq-surface p-4">
    <form className="space-y-3" onSubmit={(event) => { preventDefaultFormSubmit(event); void onSave({ allianceId: alliance.id, provider, sources, enabled, version: target?.version ?? 0 }); }}>
      <h3 className="font-semibold">{t(provider === "apple" ? "apple" : "google")}</h3><label className="flex gap-2"><input type="checkbox" disabled={!canEnable} checked={enabled} onChange={(event) => setEnabled(event.target.checked)} />{t("enable")}</label>
