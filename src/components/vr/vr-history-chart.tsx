...
export function VrHistoryChart({ events, className }: Props) {
  if (events.length < 2) {
    return null;
  }
  ...
  const minVr = Math.min(...events.map((e) => e.baseVr));
  const maxVr = Math.max(...events.map((e) => e.baseVr));
  const vrSpan = Math.max(maxVr - minVr, 250);
  ...
  const points = events.map((event, index) => {
    ...
    const y =
      PAD.top + innerH - ((event.baseVr - minVr) / vrSpan) * innerH;
    return { x, y, event };
  });
