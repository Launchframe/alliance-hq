...
export function DevQuickSwitch() {
  ...
  return (
    <div className="fixed bottom-4 left-4 z-[200] flex flex-col items-start gap-2 print:hidden">
      {open ? (
        <div className="flex max-h-[70vh] w-[19rem] flex-col overflow-hidden rounded-xl border border-amber-500/40 bg-hq-surface shadow-2xl">
          <div className="flex items-center justify-between border-b border-hq-border px-3 py-2">
            <span className="text-xs font-semibold uppercase tracking-wide text-amber-400">
              Dev quick-switch
            </span>
            <button
              type="button"
              className="rounded px-2 py-0.5 text-xs text-hq-fg-muted hover:bg-hq-surface-muted hover:text-hq-fg"
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-2 py-2">
            {groups.map((group) => (
              <div key={group.key} className="mb-3 last:mb-0">
                <p className="px-1 pb-1 text-[0.65rem] font-semibold uppercase tracking-wider text-hq-fg-subtle">
                  {group.heading}
                </p>
                  ...
                  {group.accounts.map((account) => (
                    <li key={account.email}>
                      <button
                        ...
                        className={cn(
                          "flex w-full items-center justify-between gap-2 rounded-md border border-transparent px-2 py-1.5 text-left text-xs text-hq-fg transition-colors hover:border-hq-border hover:bg-hq-surface-muted",
                      ...
                      >
                        ...
                        {account.ashed ? (
                          ...
                          <span className="shrink-0 text-[0.6rem] text-hq-fg-subtle">
                            no Ashed
                          </span>
                        )}
                      </button>
          ...
          <div className="border-t border-hq-border px-3 py-2">
            {error ? (
            ...
            <button
              ...
              disabled={busy !== null}
              onClick={handleExit}
              className="w-full rounded-md border border-hq-border px-2 py-1.5 text-xs text-hq-fg transition-colors hover:bg-hq-surface-muted disabled:opacity-60"
            >
              Exit / return to me
            </button>
          </div>
      ...
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="rounded-full border border-amber-500/50 bg-hq-surface px-3 py-1.5 text-xs font-semibold text-amber-400 shadow-lg transition-colors hover:bg-hq-surface-muted"
        aria-expanded={open}
      >
        {open ? "▾" : "▴"} Dev users
      </button>
