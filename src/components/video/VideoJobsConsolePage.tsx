...
export function VideoJobsConsolePage({
  ...
  const setFilters = useCallback(
    (patch: Partial<AdminVideoJobsListFilters>) => {
      router.replace(videoJobsListHref(config.listPath, { ...filters, ...patch }), {
        scroll: false,
      });
    },
    [filters, router, config.listPath],
  );
  ...
  async function runRequeue(jobId: string) {
    ...
    setError(null);
    try {
      const res = await fetch(`${config.apiBase}/${jobId}/requeue`, {
        method: "POST",
      });
      if (!res.ok) {
  ...
  async function confirmReprocess(body: {
    ...
    setError(null);
    try {
      const res = await fetch(`${config.apiBase}/${jobId}/reprocess`, {
        method: "POST",
            ...
            >
              ...
              <RecordDetailField label={tJobs("actions")}>
                <div className="flex flex-wrap items-center gap-2 text-sm font-normal">
                  <Link
                    href={videoJobDetailHref(config.listPath, job.id, filters)}
