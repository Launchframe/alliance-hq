  ...
  return <div className="mx-auto max-w-2xl space-y-6 p-4 sm:p-6">
    ...
    {draft && settings ? <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); void save(); }}>
      <fieldset disabled={!settings.canManage || busy || loading} className="space-y-4">
        ...
        <label className="block space-y-2">{t("preset")}<select aria-label={t("preset")} value={draft.preset} onChange={(event) => setDraft({ ...draft, preset: event.target.value as Draft["preset"] })} className={inputClass}><option value="rank_aware">{t("rankAware")}</option><option value="consecutive">{t("consecutive")}</option></select></label>
