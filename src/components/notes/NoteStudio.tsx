...
export function NoteStudio({ canCreate, onChanged }: { canCreate: boolean; onChanged: () => Promise<void> }) {
  ...
  const [kind, setKind] = useState<GenerationKind>(canCreate ? "synthesize" : "ask"), [question, setQuestion] = useState("");
  const [resources, setResources] = useState<Array<{ resourceId: string; title: string }>>([]), [selected, setSelected] = useState<string[]>([]);
  const [history, setHistory] = useState<Array<{ id: string; kind: GenerationKind }>>([]), [job, setJob] = useState<GenerationResult | null>(null);
  const [review, setReview] = useState<GenerationReview | null>(null), [dirty, setDirty] = useState(false), [thread, setThread] = useState<string | null>(null);
  const [includeSources, setIncludeSources] = useState(false), [busy, setBusy] = useState(false), [error, setError] = useState<string | null>(null);
  const currentId = useRef<string | null>(null), controller = useRef<AbortController | null>(null);
  const api = useCallback(async <T,>(url: string, body?: unknown, method = "POST"): Promise<T> => {
    const response = await fetch(url, { cache: "no-store", ...(body !== undefined ? { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) } : {}) });
  ...
  const load = useCallback(async (id: string, initialize = false) => {
    ...
    try {
      const response = await fetch(`/api/notes/generation/${id}`, { cache: "no-store", signal: active.signal });
  ...
  useEffect(() => {
    const check = () => { if (currentId.current) void load(currentId.current); };
  ...
  const choose = async (id: string) => { currentId.current = id; setReview(null); setDirty(false); await load(id, true); };
  ...
  return <section data-testid="notes-studio" className="min-w-0 flex-1 space-y-5 p-5 sm:p-7"><h2 className="text-xl font-semibold">{t("title")}</h2><p className="text-sm text-hq-fg-muted">{t("hint")}</p>
    ...
    <button className={cls} disabled={busy || dirty || kind !== "ask" && !selected.length || kind === "ask" && !question.trim()} onClick={() => void action(async () => { const result = await api<{ jobId: string }>("/api/notes/generation", { requestId: crypto.randomUUID(), kind, locale, resourceIds: selected, question, threadId: kind === "ask" ? thread : null, includeSources }); await choose(result.jobId); await refresh(); })}>{t("start")}</button>
    ...
    {canCreate && <><p className="text-xs text-hq-fg-muted">{t("unsaved")}</p><div className="flex gap-2"><button className={cls} disabled={busy || !dirty} onClick={() => void action(async () => { const saved = await api<GenerationResult>(`/api/notes/generation/${job.id}/review`, { ...review, expectedVersion: job.version, requestId: crypto.randomUUID() }); setJob(saved); setDirty(false); })}>{t("saveReview")}</button><button className={cls} disabled={busy || !review.body.trim() || !review.title.trim()} onClick={() => void action(async () => { await api(`/api/notes/generation/${job.id}`, { ...review, expectedVersion: job.version, requestId: crypto.randomUUID() }); setDirty(false); setReview(null); await load(job.id); await onChanged(); })}>{t("accept")}</button></div></>}
