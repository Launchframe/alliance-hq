...
export function NoteStudio({ canCreate, onChanged }: { canCreate: boolean; onChanged: () => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    Promise.all([api<ResourcePage<typeof resources[number]>>(`/api/notes/knowledge/resources?${libraryQuery}`), api<NonNullable<typeof history>>(`/api/notes/generation?${query}`)])
      .then(([catalog, recent]) => { if (active) { setResources(catalog.items); setCatalogNext(catalog.nextCursor); setCatalogPrevious(catalog.previousCursor); setCatalogKey(resourceKey); setHistory(recent); setHistoryKey(reviewCursor); } })
      .catch(() => { if (active) { setResources([]); setHistory(null); setError(n("loadFailed")); } });
    return () => { active = false; };
  ...
  return <section data-testid="notes-studio" className="min-w-0 flex-1 space-y-5 p-5 sm:p-7"><h2 className="text-xl font-semibold">{t("title")}</h2><p className="text-sm text-hq-fg-muted">{t("hint")}</p>
    <div className="grid gap-4 lg:grid-cols-[15rem_minmax(0,1fr)]"><aside className="space-y-2"><h3 className="text-sm font-semibold">{t("history")}</h3>{historyKey === reviewCursor && history?.items.map((item) => <button disabled={busy || dirty} className={`${cls} block w-full text-left`} key={item.id} onClick={() => void action(() => choose(item.id))}>{t(`kinds.${item.kind}`)} · {item.id.slice(-4)}</button>)}<div className="flex gap-2"><button className={cls} disabled={busy || historyKey !== reviewCursor || !history?.previousCursor} onClick={() => navigation.change({ reviewCursor: history?.previousCursor ?? null })}>{n("imports.previous")}</button><button className={cls} disabled={busy || historyKey !== reviewCursor || !history?.nextCursor} onClick={() => navigation.change({ reviewCursor: history?.nextCursor ?? null })}>{n("imports.next")}</button></div></aside><div className="space-y-4">
