...
export function NoteDraftsPanel({ onOpen, refreshKey }: { onOpen: (id: string) => void; refreshKey: boolean }) {
  ...
  return <section className="min-w-0 flex-1 space-y-4 p-6" data-testid="notes-drafts"><h2 className="font-semibold">{t("views.drafts")}</h2><p className="text-sm text-hq-fg-muted">{t("drafts.private")}</p>
    <div ref={errorAnchor}>{error && <><p role="alert" className="text-hq-danger">{error}</p><button type="button" className={button} onClick={() => setRetry((value) => value + 1)}>{t("workspace.retryLoading")}</button></>}</div>
    {current && !current.drafts.length && <p className="text-sm text-hq-fg-muted">{t("drafts.empty")}</p>}
    {current?.drafts.map((draft) => <button type="button" key={draft.id} onClick={() => onOpen(draft.id)} className="block w-full space-y-1 rounded-xl border border-hq-border bg-hq-canvas p-4 text-left"><span className="block font-medium">{draft.title || t("drafts.untitled")}</span><span className="text-xs text-hq-fg-muted">{t(`source.${draft.source}`)} · {formatAccountDate(draft.updatedAt, { locale, timezoneId: "server", dateStyle: "medium" })}</span></button>)}
    <div className="flex gap-2"><button type="button" className={button} disabled={!current?.previousCursor && !(cursor && current && !current.drafts.length)} onClick={() => navigation.change({ draftCursor: current?.previousCursor ?? null })}>{t("imports.previous")}</button><button type="button" className={button} disabled={!current?.nextCursor} onClick={() => navigation.change({ draftCursor: current?.nextCursor ?? null })}>{t("imports.next")}</button></div>
  </section>;
}
