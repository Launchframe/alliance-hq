...
export function NoteTaskEditor({ task, sourceNoteId, boardName, people, onClose, onSave }: {
  ...
  return <dialog ref={dialog} aria-label={t(task ? "tasks.edit" : "tasks.new")} onCancel={(event) => { event.preventDefault(); event.stopPropagation(); if (discard) setDiscard(false); else close(); }} className="fixed inset-0 m-auto max-h-[92dvh] w-[min(95vw,38rem)] overflow-y-auto rounded-2xl border border-hq-border bg-hq-canvas p-6 text-hq-fg shadow-2xl backdrop:bg-black/60">
    {discard ? <div className="space-y-4"><h2 className="font-semibold">{t("editor.discardTitle")}</h2><p className="text-sm">{t("editor.discardBody")}</p><div className="flex justify-end gap-2"><button type="button" onClick={() => setDiscard(false)} className="rounded border border-hq-border p-2">{t("editor.keepEditing")}</button><button type="button" onClick={onClose} className="rounded bg-hq-danger p-2 text-white">{t("editor.discard")}</button></div></div> : <div className="space-y-4">
      ...
      {latest ? <section className="space-y-3 rounded-lg border border-hq-border bg-hq-surface p-3" aria-label={t("tasks.latestVersion")}><h3 className="text-sm font-semibold">{t("tasks.latestVersion")}</h3><p className="font-medium">{latest.title}</p>{latest.description ? <p className="whitespace-pre-wrap text-sm">{latest.description}</p> : null}<p className="text-xs">{t(`tasks.status.${latest.status}`)} · {t(`priority.${latest.priority ?? "none"}`)} · {latest.assignee?.name ?? t("tasks.unassigned")}</p>{latest.dueAt ? <p className="text-xs">{formatAccountDate(latest.dueAt, { locale, timezoneId: "server", dateStyle: "medium" })}</p> : null}<p className="text-xs">{latest.labels.join(", ")}</p><p className="text-xs text-hq-fg-muted">{t("tasks.reapplyHint")}</p><button type="button" disabled={!latest.canEdit || saving} onClick={() => { setDraft((current) => ({ ...taskEditorFields(latest), ...Object.fromEntries(changedFields.map((key) => [key, current[key as keyof typeof current]])) })); setExpectedVersion(latest.version); setRequestId(crypto.randomUUID()); setLatest(null); setError(null); }} className="rounded-lg border border-hq-border px-3 py-2 text-sm">{t("tasks.keepChanges")}</button></section> : error && task ? <button type="button" disabled={saving} onClick={() => void reviewLatest()} className="text-sm text-hq-accent">{t("tasks.reviewLatest")}</button> : null}
      <footer className="sticky bottom-0 space-y-3 border-t border-hq-border bg-hq-canvas pt-3">{error ? <p role="alert" className="text-sm text-hq-danger">{error}</p> : null}<div className="flex items-center justify-between gap-2">{task?.isOwner ? <button type="button" disabled={saving} onClick={() => void save(!task.archived)} className="text-xs text-hq-fg-muted">{t(task.archived ? "tasks.restore" : "tasks.archive")}</button> : <span />}<div className="flex gap-2"><button type="button" disabled={saving} onClick={close} className="rounded-lg border border-hq-border px-3 py-2 text-sm">{t("actions.close")}</button>{editable ? <button type="button" disabled={saving || !draft.title.trim()} onClick={() => void save()} className="rounded-lg bg-hq-accent px-4 py-2 text-sm text-white disabled:opacity-50">{saving ? t("saving") : t(boardName && !task ? "boards.saveTask" : "tasks.save")}</button> : null}</div></div></footer>
    </div>}
  </dialog>;
}
