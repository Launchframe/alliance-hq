...
export function NoteKnowledge({ onChanged }: { onChanged: () => Promise<void> }) {
  ...
  const [owned, setOwned] = useState(true), [offset, setOffset] = useState(0);
  const [catalog, setCatalog] = useState<Catalog>({ resources: [], nextOffset: null });
  const [selected, setSelected] = useState<string | null>(null), [status, setStatus] = useState<KnowledgeStatus | null>(null);
  const [query, setQuery] = useState(""), [includeSources, setIncludeSources] = useState(false);
  ...
  const api = useCallback(async <T,>(url: string, body?: unknown, signal?: AbortSignal): Promise<T> => {
    ...
    if (!response.ok || !data) {
      if ([401, 403, 404].includes(response.status)) { epoch.current++; selection.current = null; current.current = null; setSelected(null); setCatalog({ resources: [], nextOffset: null }); setStatus(null); clearEvidence(); }
      throw new Error(data?.code === "rate_limited" ? t("limits") : data?.error ?? notesT("loadFailed"));
    }
    return data;
  }, [clearEvidence, notesT, t]);
  ...
  async function search(mode: "keyword" | "semantic") {
    ...
    try {
      ...
      if (!controller.signal.aborted) setEvidence(data.evidence);
  ...
  return <section data-testid="notes-knowledge" className="min-w-0 flex-1 space-y-5 p-5 sm:p-7">
    ...
    <div className="flex gap-2">{[true, false].map((value) => <button key={String(value)} disabled={busy} className={`${button} ${owned === value ? "bg-hq-accent/10 text-hq-accent" : ""}`} onClick={() => { setOwned(value); setOffset(0); setSelected(null); clearEvidence(); }}>{t(value ? "owned" : "library")}</button>)}</div>
