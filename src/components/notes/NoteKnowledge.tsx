...
export function NoteKnowledge({ onChanged }: { onChanged: () => Promise<void> }) {
  ...
  const refreshCatalog = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      ...
      if (!signal?.aborted && number === catalogRead.current) setCatalog({ resources: data.items, nextCursor: data.nextCursor, previousCursor: data.previousCursor });
    } catch (failure) { if (!signal?.aborted && number === catalogRead.current) setError(failure instanceof Error ? failure.message : notesT("loadFailed")); }
  ...
  return <section data-testid="notes-knowledge" className="min-w-0 flex-1 space-y-5 p-5 sm:p-7">
    ...
    <div className="grid gap-5 lg:grid-cols-[16rem_minmax(0,1fr)]"><aside className="space-y-2">{catalog.resources.map((resource) => <div key={resource.resourceId} className="rounded-lg border border-hq-border p-3"><p className="text-xs text-hq-fg-muted">{notesT(`searchWorkspace.${resource.kind}`)}</p>{resource.isOwner ? <button disabled={busy} onClick={() => setSelected(resource.resourceId)} className="mt-1 text-left text-sm font-medium text-hq-accent">{resource.title || notesT("editor.untitled")}</button> : resource.kind === "note" || resource.kind === "task" ? <Link href={resource.kind === "note" ? `/notes/${resource.entityId}` : `/notes?view=tasks&task=${encodeURIComponent(resource.entityId)}`} className="mt-1 block text-sm text-hq-accent">{resource.title}</Link> : <p className="text-sm">{resource.title}</p>}</div>)}{!catalog.resources.length && <p className="text-sm text-hq-fg-muted">{t("empty")}</p>}<div className="flex gap-2"><button className={button} disabled={!catalog.previousCursor || busy} onClick={() => setCursor(catalog.previousCursor)}>{t("previous")}</button><button className={button} disabled={!catalog.nextCursor || busy} onClick={() => setCursor(catalog.nextCursor)}>{t("next")}</button></div></aside>
