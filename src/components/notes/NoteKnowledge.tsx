...
export function NoteKnowledge({ onChanged }: { onChanged: () => Promise<void> }) {
  ...
  return <section data-testid="notes-knowledge" className="min-w-0 flex-1 space-y-5 p-5 sm:p-7">
    ...
    {evidence && !evidence.length && <p className="text-sm text-hq-fg-muted">{t("noEvidence")}</p>}{evidence?.map((item) => <article key={item.id} className="rounded-lg border border-hq-border p-4"><p className="mb-2 text-xs text-hq-fg-muted">{notesT(`searchWorkspace.${item.kind}`)}</p><p className="whitespace-pre-wrap break-words text-sm">{item.text}</p></article>)}
  </section>;
}
