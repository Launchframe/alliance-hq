...
export function NotePublications({ scope }: { scope: string }) {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      try {
        ...
        if (revision === mutationRevision.current) { setItems(value.items); setHistoryNext(value.nextCursor); setHistoryPrevious(value.previousCursor); setHistoryKey(historyCursor); }
  ...
  }, [id, ready, historyCursor, historyRefresh, scope, n, clear, fetchNotes]);
  ...
  async function loadSnapshot(item: PublicationSummary) {
    const target = id; setBusy(true); setError(null);
    try {
      ...
      setPreview(value); setReviewed(false);
    } catch (failure) { if (selected.current === target) setError(failure instanceof Error ? failure.message : n("loadFailed")); }
  ...
  return <section data-testid="notes-publications" className="min-w-0 flex-1 space-y-5 p-5 sm:p-7"><h2 className="text-xl font-semibold">{t("title")}</h2><p className="text-sm text-hq-fg-muted">{t("hint")}</p>
    ...
    <div className="space-y-3" data-testid="publication-history">{historyKey === historyCursor && items.filter((item) => item.noteId === id).map((item) => {
    ...
    })}<div className="flex gap-2"><button className={cls} disabled={busy || historyKey !== historyCursor || !historyPrevious} onClick={() => navigation.change({ snapshotCursor: historyPrevious })}>{n("imports.previous")}</button><button className={cls} disabled={busy || historyKey !== historyCursor || !historyNext} onClick={() => navigation.change({ snapshotCursor: historyNext })}>{n("imports.next")}</button></div></div></>}
