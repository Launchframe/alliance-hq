...
import { publicationSummary, type Publication, type PublicationSummary } from "@/lib/notes/publications.shared";
...
export function NotePublications({ scope, onRevoke }: { scope: string; onRevoke: () => void }) {
  ...
  const [preview, setPreview] = useState<Publication | null>(null), [items, setItems] = useState<PublicationSummary[]>([]), [reviewed, setReviewed] = useState(false);
  const [busy, setBusy] = useState(false), [error, setError] = useState<string | null>(null);
  const historyCursor = params.get("snapshotCursor");
  const [historyError, setHistoryError] = useState<string | null>(null), [failedSnapshot, setFailedSnapshot] = useState<PublicationSummary | null>(null);
  const historyErrorAnchor = useRef<HTMLDivElement>(null);
  useEffect(() => { if (historyError) historyErrorAnchor.current?.scrollIntoView({ block: "nearest" }); }, [historyError]);
  const [historyNext, setHistoryNext] = useState<string | null>(null), [historyPrevious, setHistoryPrevious] = useState<string | null>(null);
  const [historyKey, setHistoryKey] = useState<string | null | undefined>(undefined), [historyRefresh, setHistoryRefresh] = useState(0);
  ...
  const clear = useCallback(() => { mutationRevision.current++; selected.current = ""; setId(""); setSource(null); setTitle(""); setBody(""); setPreview(null); setItems([]); setReviewed(false); setHistoryError(null); setFailedSnapshot(null); }, []);
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      try {
        const query = new URLSearchParams({ noteId: id, format: "summary", ...(historyCursor ? { cursor: historyCursor } : {}) });
        const [detail, response] = await Promise.all([fetchNotes(`/api/notes/${encodeURIComponent(id)}`, { cache: "no-store", signal: controller.signal }), fetchNotes(`/api/notes/publications?${query}`, { cache: "no-store", signal: controller.signal })]);
        ...
        if (!detail.ok || !response.ok) { if (revision === mutationRevision.current) setHistoryError(value.error ?? note.error ?? n("loadFailed")); return; }
        if (revision === mutationRevision.current) { setItems(value.items); setHistoryNext(value.nextCursor); setHistoryPrevious(value.previousCursor); setHistoryKey(historyCursor); setHistoryError(null); }
      } catch { if (!controller.signal.aborted && revision === mutationRevision.current) setHistoryError(n("loadFailed")); }
    };
    void refresh();
    window.addEventListener("focus", refresh);
    const timer = window.setInterval(() => { void refresh(); }, 30_000);
    return () => { controller.abort(); window.removeEventListener("focus", refresh); window.clearInterval(timer); };
  }, [id, ready, historyCursor, historyRefresh, scope, n, clear, fetchNotes, onRevoke]);
  ...
  async function run(action: () => Promise<Publication>) {
    ...
    setBusy(true); setError(null); setFailedSnapshot(null);
    try { const value = await action(); if (selected.current === target) { setPreview(value); setReviewed(false); setHistoryKey(historyCursor); setHistoryRefresh((version) => version + 1); setItems((current) => current.some((item) => item.id === value.id) ? current.map((item) => item.id === value.id ? publicationSummary(value) : item) : [publicationSummary(value), ...current].slice(0, 50)); } }
  ...
  async function loadSnapshot(item: PublicationSummary) {
    const target = id; setBusy(true); setError(null); setFailedSnapshot(null);
    try {
      const response = await fetchNotes(`/api/notes/publications/${encodeURIComponent(item.id)}`, { cache: "no-store" });
      if (selected.current !== target) return;
      if ([401, 403].includes(response.status)) { clear(); onRevoke(); return; }
      const value = await response.json();
      if (selected.current !== target) return;
      if (!response.ok) throw new Error(value.error ?? n("loadFailed"));
      setPreview(value); setReviewed(false);
    } catch (failure) { if (selected.current === target) { setFailedSnapshot(item); setError(failure instanceof Error ? failure.message : n("loadFailed")); } }
    finally { setBusy(false); }
  }
  const command = (item: PublicationSummary, command: "publish" | "revoke" | "rotate") => run(() => post(`/api/notes/publications/${item.id}`, { requestId: crypto.randomUUID(), expectedVersion: item.version, command, reviewed }));
  ...
  return <section data-testid="notes-publications" className="min-w-0 flex-1 space-y-5 p-5 sm:p-7"><h2 className="text-xl font-semibold">{t("title")}</h2><p className="text-sm text-hq-fg-muted">{t("hint")}</p>
    <div className="space-y-3">
      ...
      <label className="block text-sm">{t("note")}<select aria-label={t("note")} value={wanted} disabled={busy || listing} className={`${cls} ml-3`} onChange={(event) => navigation.change({ publicationNote: event.target.value || null, snapshotCursor: null })}><option value="">—</option>{source && !notes.some((note) => note.id === id) && <option value={id}>{source.title || n("editor.untitled")}</option>}{notes.map((note) => <option value={note.id} key={note.id}>{note.title || n("editor.untitled")}</option>)}</select></label>
      ...
      {choiceError && <><p role="alert" className="text-sm text-hq-danger">{choiceError}</p><button type="button" disabled={busy || listing} className={cls} onClick={() => { void loadChoices(); if (wanted && !ready) void choose(wanted); }}>{n("workspace.retryLoading")}</button></>}
    </div>
    ...
    <div className="space-y-3" data-testid="publication-history">{historyError && <div ref={historyErrorAnchor} className="space-y-2"><p role="alert" className="text-sm text-hq-danger">{historyError}</p><button type="button" disabled={busy} className={cls} onClick={() => { setHistoryError(null); setHistoryRefresh((version) => version + 1); }}>{n("workspace.retryLoading")}</button></div>}{historyKey === historyCursor && items.filter((item) => item.noteId === id).map((item) => {
      const link = preview?.id === item.id ? preview.link : null;
      return <article key={item.id} className="space-y-2 rounded-lg border border-hq-border p-4"><button type="button" disabled={busy} onClick={() => void loadSnapshot(item)} className="text-left font-semibold">{item.title} · {t("version", { version: item.snapshotVersion })}</button><p className="text-xs">{t(`states.${item.state}`)} · {t("expires", { date: new Date(item.expiresAt).toLocaleString(locale) })}</p><div className="flex flex-wrap gap-2">{link && <><a className={cls} href={link} target="_blank" rel="noopener noreferrer">{t("open")}</a><button type="button" className={cls} onClick={() => void navigator.clipboard.writeText(new URL(link, window.location.origin).href).catch(() => setError(n("saveFailed")))}>{t("copy")}</button></>}{item.state === "published" && <button type="button" className={cls} disabled={busy} onClick={() => void command(item, "rotate")}>{t("rotate")}</button>}{item.state !== "revoked" && <button type="button" className={cls} disabled={busy} onClick={() => void command(item, "revoke")}>{t("revoke")}</button>}</div></article>;
    })}<div className="flex gap-2"><button type="button" className={cls} disabled={busy || historyKey !== historyCursor || !historyPrevious} onClick={() => navigation.change({ snapshotCursor: historyPrevious })}>{n("imports.previous")}</button><button type="button" className={cls} disabled={busy || historyKey !== historyCursor || !historyNext} onClick={() => navigation.change({ snapshotCursor: historyNext })}>{n("imports.next")}</button></div></div></>}
    {error && <p ref={errorAnchor} role="alert" className="text-sm text-hq-danger">{error}</p>}
    {failedSnapshot && <button type="button" disabled={busy} className={cls} onClick={() => void loadSnapshot(failedSnapshot)}>{n("workspace.retryLoading")}</button>}
  </section>;
}
