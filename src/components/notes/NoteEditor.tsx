      ...
      <div className="min-h-0 overflow-y-auto">
        ...
        <div className="grid gap-6 px-6 py-6 sm:grid-cols-[minmax(0,1fr)_15rem] sm:px-8">
          ...
          <aside className="space-y-4 rounded-xl bg-hq-surface p-4 sm:self-start">
            ...
            <label className="block space-y-1.5"><span className="text-xs font-medium text-hq-fg-muted">{t("fields.labels")}</span><input value={labels} disabled={!editable || saving} onChange={(event) => setLabels(event.target.value)} placeholder={t("editor.labelsPlaceholder")} className={inputClass} /></label>
            {owner ? <label className="block space-y-1.5"><span className="text-xs font-medium text-hq-fg-muted">{t("fields.notebook")}</span><input value={draft.notebook} disabled={!editable || saving} maxLength={60} onChange={(event) => setDraft({ ...draft, notebook: event.target.value })} placeholder={t("editor.noNotebook")} className={inputClass} /></label> : null}
