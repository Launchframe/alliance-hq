...
import { NOTE_PRIORITIES, noteListUrl, readNoteListFilter, notesWorkspaceLocation, type NoteFields, type NotePatch, type NoteWorkspaceView } from "@/lib/notes/workspace.shared";
...
export function NotesClient({ initial, focusedNote, initialCursor = null }: { initial: NotesWorkspacePayload; focusedNote?: PerformanceNoteDto | null; initialCursor?: string | null }) {
  ...
  const [notebook, setNotebook] = useState(initial.filter.notebook);
  const [query, setQuery] = useState(initial.filter.q);
  const [source, setSource] = useState<string>(initial.filter.source);
  const [priority, setPriority] = useState<string>(initial.filter.priority);
  ...
  const refresh = useCallback(async (next?: string | null) => {
    ...
    const cursor = next === undefined ? pagePosition.current.key === filterKey ? pagePosition.current.cursor : null : next;
    setLoading(true);
    try {
  ...
  }, [filter, filterKey, initial.scope, revoke, t]);
  ...
  async function openNote(summary: PerformanceNoteSummary, kind: "editor" | "share" = "editor") {
    ...
    try {
      ...
      url.searchParams.set("note", summary.id);
  ...
  async function save(fields: NoteFields | NotePatch | CaptureCommit, noteId?: string) {
    const response = await fetch(noteId ? `/api/notes/${noteId}` : "tasks" in fields ? "/api/notes/capture" : "/api/notes?format=summary", {
