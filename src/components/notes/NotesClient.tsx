...
export function NotesClient({ initial, focusNoteId }: { initial: PerformanceNotesPagePayload; focusNoteId?: string }) {
  ...
  const counts = useMemo(() => ({
    tasks: undefined, boards: undefined, drafts: data.draftCount, imports: undefined, search: undefined, knowledge: undefined, studio: undefined, publications: undefined,
  ...
  return <main className="min-h-[calc(100dvh-4rem)] min-w-0 bg-hq-canvas" data-testid="notes-workspace">
    ...
    <div className="flex min-w-0">
      ...
      {view === "publications" ? <NotePublications notes={data.notes} /> : view === "studio" ? <NoteStudio canCreate={data.canCreate} onChanged={refresh} /> : view === "knowledge" ? <NoteKnowledge onChanged={refresh} /> : view === "search" ? <NoteWorkspaceSearch /> : view === "imports" ? <NoteHistoryImports canCreate={data.canCreate} focusId={importId} onOpen={(id) => {
