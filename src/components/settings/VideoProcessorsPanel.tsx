  ...
  return (
    <section className="space-y-3 rounded-xl border border-hq-border bg-hq-surface p-4">
      <div>
        ...
        <p className="mt-1 text-xs text-hq-fg-muted">{t(descriptionKey, { max })}</p>
        ...
        {readOnly ? (
          <p className="mt-2 text-xs text-hq-fg-muted">{t("readOnlyHint")}</p>
        ) : null}
      </div>
      ...
      {processors.length === 0 ? (
        <p className="text-sm text-hq-fg-subtle">{t("empty")}</p>
      ) : (
            ...
            >
              ...
              {!readOnly ? (
                <button
                  ...
                  className="shrink-0 rounded-md border border-hq-border px-2.5 py-1 text-xs text-hq-fg-muted hover:border-hq-danger hover:text-hq-danger disabled:cursor-not-allowed disabled:opacity-50"
                >
                  {t("remove")}
                </button>
              ) : null}
      ...
      {!readOnly && slotsFull ? (
        ...
        <p className="text-xs text-hq-fg-subtle">{t(noCandidatesKey)}</p>
      ) : !readOnly ? (
        <div className="flex flex-col gap-2 sm:flex-row">
          <select
            value={selected}
            onChange={(e) => setSelected(e.target.value)}
            className="min-w-0 flex-1 rounded-lg border border-hq-border bg-hq-canvas px-2 py-1.5 text-sm text-hq-fg"
          >
          ...
          <button
            ...
            onClick={() => void add()}
            className="shrink-0 rounded-lg border border-hq-green px-3 py-1.5 text-sm font-medium text-hq-green hover:bg-[#3fb95020] disabled:cursor-not-allowed disabled:opacity-50"
          >
            {t("add")}
          </button>
        </div>
