...
export function PlunderPlanCalendarClient({ initial, initialDate }: { initial: PlanDashboard; initialDate: string }) {
  ...
  const eventTitle = (event: CalendarPlan) => `${event.kind === "suggestion" ? t("suggestionLabel") : event.memberName}: ${timeLabel(event.startAt)}${event.kind === "regular" ? "" : `–${timeLabel(event.endAt)}`}`;
  ...
  return <main className="mx-auto max-w-7xl space-y-5 p-3 text-hq-fg sm:p-6">
    ...
    <div className="flex flex-wrap gap-2"><button className={button} disabled={!data.canManageSelf || !data.commanders.length} onClick={() => open({ kind: "edit" })}>{t("add")}</button><button className={button} disabled={!data.canManageSelf} onClick={() => { setColor(data.color); open({ kind: "color" }); }}>{t("color.title")}</button>{data.canSuggest && <button className={button} onClick={() => open({ kind: "edit", suggestion: true })}>{t("suggest")}</button>}{data.canSuggest && <button className={button} onClick={() => open({ kind: "notifications" })}>{t("notifications.title")}</button>}</div>
    ...
    <dialog ref={dialog} aria-label={t("title")} className="max-h-[90vh] w-[min(95vw,38rem)] overflow-auto rounded-xl border border-hq-border bg-hq-canvas p-5 text-hq-fg backdrop:bg-black/60" onCancel={(event) => { if (busy) event.preventDefault(); else setModal(null); }}>
      ...
      {modal?.kind === "event" && <div className="space-y-3"><h2 className="text-lg font-semibold">{eventTitle(modal.event)}</h2><p>{new Intl.DateTimeFormat(locale, { dateStyle: "full", timeStyle: "short", timeZone: zone }).format(new Date(modal.event.startAt))}</p><p>{t("serverTime")}: {new Intl.DateTimeFormat(locale, { dateStyle: "medium", timeStyle: "short", timeZone: "Etc/GMT+2" }).format(new Date(modal.event.startAt))}</p>{selectedPlan?.kind === "suggestion" && <><p>{t("suggestionHint")}</p><button className={button} disabled={!data.canManageSelf || !data.commanders.length} onClick={() => open({ kind: "edit", source: selectedPlan })}>{t("join")}</button>{data.canSuggest && <button className={button} onClick={() => open({ kind: "edit", plan: selectedPlan, suggestion: true })}>{t("editSuggestion")}</button>}{data.canSuggest && <button className={button} onClick={() => open({ kind: "remove", plan: selectedPlan })}>{t("removeSuggestion")}</button>}</>}{selectedPlan?.owned && selectedPlan.kind === "plan" && <div className="flex flex-wrap gap-2"><button className={button} onClick={() => open({ kind: "edit", plan: selectedPlan })}>{t("edit")}</button>{selectedPlan.schedule.kind === "weekly" && <button disabled={busy} className={button} onClick={() => void save({ action: "skip", id: selectedPlan.id, expectedVersion: selectedPlan.version, date: modal.event.localDate }, t("skipped"))}>{t("skip")}</button>}</div>}</div>}
