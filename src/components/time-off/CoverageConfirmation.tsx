...
export function CoverageDescription({ conflict }: { conflict: CoverageConflict }) {
  ...
  return <p>{t("coverageConflict", { member: conflict.memberName, duty: conflict.dutyRole === "engineer" ? professions("engineer") : trains(conflict.dutyRole), date })}</p>;
}
...
export function useCoverageFetch() {
  ...
  const coverageDialog = <Dialog zIndex={300} open={conflicts !== null} onOpenChange={(open) => { if (!open) finish(null); }} title={t("keep")} data-testid="coverage-confirmation">
    <div className="space-y-4 p-4">
      ...
      {conflicts?.map((conflict) => <CoverageDescription key={`${conflict.assignmentId}:${conflict.dutyRole}`} conflict={conflict} />)}
      <p>{t("keepHint")}</p>
