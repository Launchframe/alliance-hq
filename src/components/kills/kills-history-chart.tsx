...
import { killsChartYDomain } from "@/lib/kills/my-kills-chart.shared";
...
export function KillsHistoryChart({ events, className }: Props) {
  if (events.length < 2) {
    return null;
  }
  ...
  const { min: minKills, max: maxKills, span: killsSpan } = killsChartYDomain(
  ...
  const points = events.map((event, index) => {
    ...
    const y = PAD.top + innerH - ((event.total - minKills) / killsSpan) * innerH;
    return { x, y, event };
  });
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="Kills over time"
      data-testid="my-kills-history-chart"
    >
      <line
        x1={PAD.left}
        y1={PAD.top + innerH}
