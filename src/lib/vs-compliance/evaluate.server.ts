...
export async function evaluateAllianceVsComplianceForWeek(input: {
  ...
  for (const member of roster) {
    ...
    // Main time-off rows do not yet carry activity_scope (Ashed sync PR);
    // treat HQ calendar entries as covering all activities ("all").
    const excused = isVsWeekExcusedByTimeOff(
      memberTimeOff.map((entry) => ({
        startDate: entry.startDate,
        endDate: entry.endDate,
        activityScope: "all" as const,
        availability: entry.availability as
          | "full_away"
          | "limited"
          | "minimums"
          | "hit_and_miss",
        entryKind: entry.entryKind as
          | "planned"
          | "officer_marked"
          | "unexpected",
      })),
