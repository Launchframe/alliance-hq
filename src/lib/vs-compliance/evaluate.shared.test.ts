...
describe("vs-compliance/evaluate.shared", () => {
  ...
  describe("timeOffEntryExcusesVsWeek / isVsWeekExcusedByTimeOff", () => {
    ...
    it("does not excuse unless availability is full_away", () => {
      for (const availability of ["limited", "minimums", "hit_and_miss"] as const) {
        expect(
          timeOffEntryExcusesVsWeek(
            timeOffEntry({ availability }),
            weekStartMonday,
            weekEndSaturday,
          ),
        ).toBe(false);
    ...
    it("excuses when availability is full_away", () => {
      expect(
        timeOffEntryExcusesVsWeek(
          timeOffEntry({ availability: "full_away" }),
          weekStartMonday,
          weekEndSaturday,
        ),
      ).toBe(true);
