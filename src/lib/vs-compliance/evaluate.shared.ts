 ...
 * ("vs" or "all" scope), and the member is fully away
 * (`availability === "full_away"`). Partial availability (limited,
 * minimums, hit_and_miss) does not excuse the weekly VS minimum.
 */
export function timeOffEntryExcusesVsWeek(
  ...
  if (entry.availability !== "full_away") {
    return false;
  }
