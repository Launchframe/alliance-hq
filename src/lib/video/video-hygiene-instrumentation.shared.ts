...
/** Job statuses where opening the review UI should stamp reviewOpenedAt. */
export const VIDEO_JOB_REVIEW_OPEN_STATUSES = ["review", "failed"] as const;

export function canStampVideoJobReviewOpened(status: string): boolean {
  return (VIDEO_JOB_REVIEW_OPEN_STATUSES as readonly string[]).includes(status);
}
