...
export type LoadVideoJobsAnalyticsOptions = {
  scoreTarget: string | null;
  passKey: string | null;
  days: number;
  /** When set, restrict aggregates to one alliance (tools processor console). */
...
export async function loadVideoJobsAnalytics(
  ...
  const bucketRows = await db
    ...
    .where(
      and(
        ...
        ...(allianceCondition ? [allianceCondition] : []),
      ),
    )
    .groupBy(
  ...
  const recAccConditions = [
    ...
    ...(days > 0
      ...
      : []),
    ...(uploadGroupAllianceCondition ? [uploadGroupAllianceCondition] : []),
  ].filter(Boolean) as Parameters<typeof and>;
