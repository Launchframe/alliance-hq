/**
 * Shared My Kills chart / stale-report helpers (client-safe).
 */

export const KILLS_STALE_REPORT_MS = 7 * 24 * 60 * 60 * 1000;

export type KillsChartPoint = {
  total: number;
  createdAt: string;
};

/** Pad the y-domain so min/max are not flush with the plot edges (avoids looking like "0"). */
export function killsChartYDomain(
  totals: number[],
  padRatio = 0.15,
 ...
 */
export function computeKillsTotalGrowth(events: KillsChartPoint[]): number | null {
...
export function isKillsReportStale(
  ...
  return nowMs - reportedMs >= KILLS_STALE_REPORT_MS;
}

export function resolveKillsLastReportedAt(input: {
