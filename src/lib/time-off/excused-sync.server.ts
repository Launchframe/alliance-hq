...
import { setTimeOffEntryAshedExcusedIds } from "@/lib/time-off/repository.server";
...
async function resolveActorAshedContext(input: {
  allianceId: string;
  sessionId?: string | null;
  discordUserId?: string | null;
}): Promise<AshedSyncContext | null> {
  if (input.sessionId) {
    return resolveWebAshedSyncContext({
      allianceId: input.allianceId,
      sessionId: input.sessionId,
    });
  }
  if (input.discordUserId) return resolveBotAshedSyncContext(input.allianceId);
  return null;
}

/**
 * Optional dual-write after an HQ save. Returns true when Ashed was expected
 * but the push/delete failed. Unexpected absences are HQ-only (not excused).
 */
export async function dualWriteTimeOffToAshed(input: {
  allianceId: string;
  entryId: string;
  sessionId?: string | null;
  discordUserId?: string | null;
  ...
  const ctx = await resolveActorAshedContext(input);
  if (!ctx) return false;
  ...
  try {
    ...
    const activityScope = isTimeOffActivityScope(row.activityScope)
      ? row.activityScope
      : "all";
    if (row.ashedExcusedIds?.length) {
      await deleteTimeOffEntryFromAshed(ctx.connection, row.ashedExcusedIds);
    }
    const ids = await pushTimeOffEntryToAshed({
    ...
    console.error("[time-off] Ashed dual-write failed", error);
    return true;
  }
}
