...
export async function findCoverageConflicts(tx: AvailabilityTransaction, allianceId: string, duties: Duty[]): Promise<CoverageConflict[]> {
  ...
  for (const duty of duties) {
    const notices = await tx.select({ id: schema.memberTimeOff.id, version: schema.memberTimeOff.version })
      .from(schema.memberTimeOff).where(and(eq(schema.memberTimeOff.allianceId, allianceId), eq(schema.memberTimeOff.ashedMemberId, duty.memberId),
        eq(schema.memberTimeOff.globalAbsence, true), isNull(schema.memberTimeOff.cancelledAt), lte(schema.memberTimeOff.startDate, duty.dutyDate), gte(schema.memberTimeOff.endDate, duty.dutyDate)));
    if (notices.length) conflicts.push({ ...duty, absenceVersion: createHash("sha256").update(JSON.stringify(notices.sort((a, b) => a.id < b.id ? -1 : a.id > b.id ? 1 : 0).map((n) => [n.id, n.version]))).digest("hex") });
  }
  return conflicts;
}
...
export async function professionCoverageDuties(tx: AvailabilityTransaction, allianceId: string, date: string): Promise<Duty[]> {
  const rows = await tx.select({ id: schema.wlEngAssignments.id, version: sql<string>`${schema.wlEngAssignments}.xmin::text`, memberId: schema.commanderAllianceMemberships.ashedMemberId, memberName: schema.commanders.primaryName })
  ...
  await withCoverageActor({ ...actor, acceptance }, () => getDb().transaction(async (tx) => {
    ...
    for (const accepted of acceptance.conflicts) {
      if (accepted.dutyRole === "engineer") {
        ...
        const duties = (await professionCoverageDuties(tx, actor.allianceId, accepted.dutyDate)).filter((duty) => duty.assignmentId === accepted.assignmentId);
