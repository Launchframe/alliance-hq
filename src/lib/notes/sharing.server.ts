...
export async function loadResourceSharing(actor: KnowledgeActor, resourceId: string): Promise<NoteShareState> {
  ...
  return { version: resource.version, allianceId: actor.allianceId, recipients, grants: grants.filter((grant): grant is NoteShareInput["grants"][number] => grant.subjectKind === "user" || grant.subjectKind === "officers") };
}
