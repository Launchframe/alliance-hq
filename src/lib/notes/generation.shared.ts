...
export const generationAcceptSchema = z.object({ requestId: z.string().min(8).max(120), expectedVersion: z.number().int().positive(), title: z.string().trim().min(1).max(160), body: z.string().trim().min(1).max(100_000), actions: z.array(captureTaskSchema).max(120).refine((actions) => actions.filter((action) => action.included).length <= 10) });
