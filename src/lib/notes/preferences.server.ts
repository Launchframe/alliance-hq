...
import { noteWorkspaceStateSchema, workspacePreferenceWriteSchema, readWorkspaceState, clampWorkspaceBoardsView, noteFilterFromWorkspace, parseNoteListCursor, type WorkspacePreferences } from "./workspace.shared";
...
export async function readWorkspacePreferences(actor: KnowledgeWebActor): Promise<WorkspacePreferences> {
  ...
  const state = clampWorkspaceBoardsView(noteWorkspaceStateSchema.parse(row?.state ?? {}), actor.canReadBoards);
...
export async function loadWorkspaceQuery(actor: KnowledgeWebActor, params: URLSearchParams) {
  ...
  try {
    const state = clampWorkspaceBoardsView(readWorkspaceState(params, preferences.state, preferences.scope), actor.canReadBoards);
    const foreign = params.has("workspaceScope") && params.get("workspaceScope") !== preferences.scope;
    return { preferences: { ...preferences, state }, filter: noteFilterFromWorkspace(state), cursor: foreign ? null : parseNoteListCursor(params.get("cursor")) };
...
export async function saveWorkspacePreferences(actor: KnowledgeWebActor, raw: unknown): Promise<WorkspacePreferences> {
  ...
  const state = clampWorkspaceBoardsView(input.state, actor.canReadBoards);
  return getDb().transaction(async (tx) => {
    ...
    const result = { scope, version: input.expectedVersion + 1, state };
