...
import { isPlaceholderOnlySearchQuery, noteSearchSchema, type NoteSearchInput, type NoteSearchResponse } from "./search.shared";
...
export async function searchNotes(actor: KnowledgeWebActor, raw: NoteSearchInput): Promise<NoteSearchResponse> {
  ...
  if (isPlaceholderOnlySearchQuery(query)) return { nextOffset: null, results: [] };
  ...
  const includeSource = actor.isOfficer && (input.kind === "all" || input.kind === "source");
  const rows = await getDb().execute(sql`
    select * from (
      ...
      from performance_notes n join knowledge_resources r on r.id = n.resource_id and r.alliance_id = n.alliance_id and r.kind = 'note' and r.entity_id = n.id
      ...
      from officer_action_items a join knowledge_resources r on r.id = a.resource_id and r.alliance_id = a.alliance_id and r.kind = 'task' and r.entity_id = a.id
      ...
      from officer_chat_messages m join officer_chat_sessions s on s.id = m.session_id and s.alliance_id = m.alliance_id
        join knowledge_resources r on r.id = s.resource_id and r.alliance_id = s.alliance_id and r.kind = 'source' and r.entity_id = s.id
      where ${includeSource} and s.alliance_id = ${actor.allianceId} and s.status = 'imported' and m.history_included and r.archived_at is null
  ...
  return { nextOffset: rows.length > input.limit && input.offset + input.limit <= 5_000 ? input.offset + input.limit : null, results: rows.slice(0, input.limit).map((row) => {
    ...
    return { id: String(row.id), kind, title: redactIntakeText(String(row.title)), excerpt: redactIntakeText(String(row.excerpt)),
      href: kind === "note" ? `/notes/${row.id}` : kind === "task" ? `/notes?view=tasks&task=${encodeURIComponent(String(row.id))}` : `/officer-intel/sessions/${row.source_id}`,
