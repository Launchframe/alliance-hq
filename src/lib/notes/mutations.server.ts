...
export async function withKnowledgeReceipt(actor: KnowledgeActor & { sessionId?: string }, action: string, requestId: string, payload: unknown, operation: (tx: KnowledgeTransaction, receiptId: string) => Promise<ReceiptResult>) {
  ...
  if (!result.replayed) await writeOfficerActionAudit({ sessionId: actor.sessionId, hqUserId: actor.hqUserId, allianceId: actor.allianceId, action, severity: ["notes.publication_revoke", "notes.publication_rotate", "notes.task_update", "notes.board_command", "notes.knowledge_unapprove", "notes.knowledge_deny_ai", "notes.knowledge_cancel"].includes(action) ? "update" : "routine", permission: action.startsWith("notes.publication_") ? action === "notes.publication_revoke" ? "notes:read" : "notes:publish" : action.startsWith("notes.board_") ? "notes_boards:write" : ["notes.generation_ask", "notes.generation_review", "notes.task_update", "notes.knowledge_unapprove", "notes.knowledge_deny_ai", "notes.knowledge_cancel"].includes(action) ? "notes:read" : "notes:create", resourceType: result.publicationId ? "note_publication" : result.resourceId ? "knowledge_resource" : "notes_workspace", resourceId: result.publicationId ?? result.resourceId });
  return result;
}
