...
describe("pickHeaderTotal", () => {
  ...
  it("prefers agreeing focused-row passes over a wrong full-column reading", () => {
    const labels: NormalizedGeometryLine[] = [
      { text: "(BD) HerolPowers", yNorm: 0.293, yCenterPx: 293 },
    ];
  ...
  it("fails closed when focused-row passes disagree", () => {
    const labels: NormalizedGeometryLine[] = [
      { text: "(BD) HerolPowers", yNorm: 0.293, yCenterPx: 293 },
    ];
    ...
    const result = pickHeaderTotal(
      ...
      [
        { lines: [ocrLine("17979827025", 10)], cropHeight: 100 },
        { lines: [ocrLine("12979827025", 10)], cropHeight: 100 },
      ],
      [],
      0,
      [],
  ...
  it("falls back to the aligned full-column row when focused OCR is empty", () => {
    const labels: NormalizedGeometryLine[] = [
      { text: "(BD) HerolPowers", yNorm: 0.293, yCenterPx: 293 },
    ];
