...
async function readableActionItems(rows: Array<typeof schema.officerActionItems.$inferSelect>, names: Map<string, string>, actor: KnowledgeActor) {
  ...
  return rows.map((row) => mapActionItemRow({ ...row, noteId: row.noteId && readableNotes.has(row.noteId) ? row.noteId : null, sessionId: row.sessionId && readableSources.has(row.sessionId) ? row.sessionId : null }, names));
}
