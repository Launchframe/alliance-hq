...
export const historyKindLabels = { createTeam: "supportTeams.addLead", replaceLead: "supportTeams.replaceLead", rename: "supportTeams.rename", move: "supportTeams.moveMember", swap: "supportTeams.swapMembers", undo: "supportTeams.history.undo", reconcile: "supportTeams.saved", scheduleDraft: "supportTeams.draft.create", draftPick: "supportTeams.draft.pick", advanceDraft: "supportTeams.draft.title", extendDraft: "supportTeams.draft.extend", publishDraft: "supportTeams.draft.publish", cancelDraft: "timeOff.officerModal.cancel", createProposal: "supportTeams.proposals.create", moveProposal: "supportTeams.moveMember", swapProposal: "supportTeams.swapMembers", submitProposal: "supportTeams.proposals.submit", approveProposal: "supportTeams.proposals.approve", publishProposal: "supportTeams.proposals.publish", cancelProposal: "timeOff.officerModal.cancel" } as const satisfies Record<SupportEvent["kind"], string>;
export function historyServiceLabel(event: SupportEvent) {
  return event.actorType === "service" || event.principalType === "service" || event.principalId.startsWith("service:") ? event.context.mode === "draft" ? "supportTeams.draft.title" : event.context.mode === "proposal" ? "supportTeams.proposals.title" : "supportTeams.title" : null;
}
...
export function humanizePatch(patch: SupportPatch, names: ReturnType<typeof historyNames>, labels: { teamName: string; lead: string; member: string; unknown: string; yes: string; no: string }, locale: string) {
  ...
  const label = resource === "member" || resource.startsWith("draftMember:") || resource.startsWith("proposalMember:") ? names.member(id) : resource === "team" || resource.startsWith("draftTeam:") || resource.startsWith("proposalTeam:") ? `${names.team(id)} · ${field === "lead" ? labels.lead : labels.teamName}` : labels.unknown;
